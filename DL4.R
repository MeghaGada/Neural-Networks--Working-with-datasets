...
require(neuralnet)
library(neuralnet)
nn5 = neuralnet(Placed~TKS+CSS,data=df,rep = 5, act.fct="logistic",linear.output = FALSE)
plot(nn5)

nn_backprop1 <- neuralnet(Placed~TKS+CSS,data=df,rep = 5, algorithm="backprop",learningrate=0.0001)
plot(nn_backprop1)

nn6 = neuralnet(Placed~TKS+CSS,data=df,rep = 5, act.fct="logistic",linear.output = FALSE)
plot(nn6)
